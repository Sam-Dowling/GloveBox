...
test.describe('folder ingest', () => {
  ...
  // ── Regression: drop with directory entry must NOT touch DataTransfer.files
  // On Firefox / Windows specifically, accessing `DataTransfer.files`
  // for a directory drop triggers a synchronous shell-call that — on a
  // cold FS cache — pushes the content process past its watchdog and
  // the tab dies with "Gah, your tab just crashed" before `_handleFiles`
  // ever runs. The fix in `app-core.js`'s drop handler skips the
  // `dt.files` read entirely whenever the items list contains any
  // directory entry; the `fsEntries` side-channel carries everything
  // we need to dispatch. Pin that with a synthetic drop event whose
  // `dataTransfer.files` is an instrumented getter — the assertion is
  // simply "the getter was never called".
  test('directory drop does NOT read DataTransfer.files (FF/Windows cold-cache crash guard)', async ({ page }) => {
    const result = await page.evaluate(async () => {
      const w = window as unknown as {
        app: {
          _handleFiles(files: unknown, opts?: { fsEntries?: unknown[] }): void;
      ...
      // Mock a directory entry the App's drop handler will receive
      // through `webkitGetAsEntry()`. The walker isn't actually run
      // here — we stub `_handleFiles` to capture and return — so the
      // reader can be a no-op (drained on first call).
      const dirEntry = {
        isFile: false,
        isDirectory: true,
        name: 'examples',
        createReader() {
          return { readEntries(ok: (e: unknown[]) => void) { ok([]); } };
        },
      };
      // Synthetic DataTransfer mirroring the surface the drop handler
      // touches: `.types` (for `isExternalFileDrag`), `.items` (for
      // the entry walk), and `.files` (the property we MUST NOT read).
      let filesReadCount = 0;
      const fakeDT = {
        types: ['Files'],
        items: [{
          kind: 'file',
          webkitGetAsEntry() { return dirEntry; },
        }],
        get files() {
          filesReadCount++;
          return { length: 0 } as unknown as FileList;
        },
      };
      // Spy on `_handleFiles` so the test can assert what the drop
      // handler routed to it without actually triggering the ingest
      // pipeline (which would call back into the spy with side effects).
      let receivedFiles: unknown = 'NOT_CALLED';
      let receivedOpts: { fsEntries?: unknown[] } | null = null;
      w.app._handleFiles = function (files: unknown, opts?: { fsEntries?: unknown[] }) {
        receivedFiles = files;
        receivedOpts = opts || null;
      };
      // Build a `drop` event whose `dataTransfer` is our fake. `Event`
      // is fine here — the handler reads `e.dataTransfer` not
      // `e instanceof DragEvent`.
      const ev = new Event('drop', { bubbles: true, cancelable: true });
      Object.defineProperty(ev, 'dataTransfer', { value: fakeDT });
      window.dispatchEvent(ev);
      return {
        receivedFiles,
        // `receivedOpts` may be null if the handler bailed before reaching
        // `_handleFiles` — the assertion below catches that case too.
        receivedOptsHasEntries: !!(receivedOpts && Array.isArray(receivedOpts.fsEntries) && receivedOpts.fsEntries.length === 1),
        filesReadCount,
      };
    });
    // M1b: when fsEntries has any directory, the drop handler passes
    // `null` to `_handleFiles` instead of `dt.files`.
    expect(result.receivedFiles).toBeNull();
    expect(result.receivedOptsHasEntries).toBe(true);
    // The crucial assertion: `dt.files` was never read for a directory
    // drop. A regression here is the FF/Windows tab-crash trigger.
    expect(result.filesReadCount).toBe(0);
  });
