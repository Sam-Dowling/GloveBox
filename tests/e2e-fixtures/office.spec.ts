...
test.describe('Office (OOXML / CFB / OpenDocument)', () => {
  ...
  test('benign .docx parses cleanly', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.docx');
    expect(findings.iocTypes).toContain('URL');
  });

  test('macro-bearing .docm fires Office_Macro_Project_Present', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.docm');
    expect(ruleNames(findings)).toContain('Office_Macro_Project_Present');
    expect(isRiskAtLeast(findings.risk, 'high')).toBe(true);
  });

  test('macro-bearing .xlsm fires Office_Macro_Project_Present', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.xlsm');
    expect(ruleNames(findings)).toContain('Office_Macro_Project_Present');
    expect(isRiskAtLeast(findings.risk, 'high')).toBe(true);
  });

  test('macro-bearing .pptm fires Office_Macro_Project_Present + PPAM_PPTM_AddIn', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.pptm');
  ...
  test('benign .xlsx is clean', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.xlsx');
    expect(findings.iocCount).toBe(0);
    expect(findings.risk).toBe('low');
  });

  test('benign .pptx is clean', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.pptx');
    expect(findings.iocCount).toBe(0);
    expect(findings.risk).toBe('low');
  });

  test('legacy .doc fires MSI_Network_Indicators / Embedded_ZIP rules', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.doc');
    const rules = ruleNames(findings);
  ...
  test('legacy .ppt fires Embedded_Compressed_Stream', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.ppt');
    expect(ruleNames(findings)).toContain('Embedded_Compressed_Stream');
  });

  test('legacy .xls (clean) parses with no IOCs', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.xls');
    expect(findings.iocCount).toBe(0);
    expect(findings.risk).toBe('low');
  });

  test('OpenDocument .odt fires Embedded_Compressed_Stream', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.odt');
    expect(ruleNames(findings)).toContain('Embedded_Compressed_Stream');
  });

  test('OpenDocument .ods fires Embedded_Compressed_Stream', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.ods');
    expect(ruleNames(findings)).toContain('Embedded_Compressed_Stream');
  });

  test('OpenDocument .odp fires Embedded_Compressed_Stream', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.odp');
    expect(ruleNames(findings)).toContain('Embedded_Compressed_Stream');
  });

  test('IQY (Excel web query) escalates to critical', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.iqy');
    expect(isRiskAtLeast(findings.risk, 'critical')).toBe(true);
    expect(ruleNames(findings)).toContain('IQY_Web_Query_File');
  });

  test('SLK (symbolic link) escalates and fires SLK rule', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.slk');
    expect(isRiskAtLeast(findings.risk, 'high')).toBe(true);
    expect(ruleNames(findings)).toContain('SLK_Symbolic_Link_File');
  });

  test('RTF fires Standalone_RTF_Obfuscation', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.rtf');
    expect(ruleNames(findings)).toContain('Standalone_RTF_Obfuscation');
    expect(isRiskAtLeast(findings.risk, 'medium')).toBe(true);
  });

  test('CSV routes to Timeline (no findings panel)', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.csv');
    expect(findings.iocCount).toBe(0);
  ...
  test('TSV routes to Timeline (no findings panel)', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/office/example.tsv');
    expect(findings.iocCount).toBe(0);
    expect(findings.risk).toBeNull();
    const result = await dumpResult(page);
    expect(result).not.toBeNull();
