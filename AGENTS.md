...
- `17e612c` — Phase 3 Python decoder fill: 4 new branches (P7 `pickle.loads(b64)` RCE primitive with pickle-opcode first-byte gate, P8 two-shape lambda-wrapped IIFE + named-alias exec/eval/compile, P9 single-byte XOR list-comp decode against literal int + literal bytestring, P10 `bytes.fromhex().decode()` hex-alphabet exec wrapper); lambda regex must use `[\s\S]` (not `[^)]`) so the body can cross quoted `)` inside payload literals; `(b?['"])…\N` back-reference demands matching `b'` suffix — move the `b` prefix OUTSIDE the capture group so `\N` only references the quote char; `SENSITIVE_PY_KEYWORDS` deliberately excludes shell-LOLBin vocab (`curl`/`wget`/`nc`) — those belong to bash/cmd gates, so XOR decodes of `curl http://x|sh` won't surface (encode Python-native sinks like `os.system` instead).
- `988d53b` — Phase 4 PHP decoder fill: 3 new branches (PHP7 `create_function('', $code)` legacy RCE plaintext-body form gated on `SENSITIVE_PHP_KEYWORDS` so `return $a+$b` style is suppressed; PHP8 `$GLOBALS['…'](…)` callable-variable indirection accepting both dangerous-fn-name keys AND superglobal-name keys for user-input dispatch — distinct from PHP5's bare `$_GET[…]()` regex so no collision; PHP9 backtick `\`…\`` shell-exec operator with two gates — PHP document context `<?` OR local PHP sigil `$var =`/`echo`/`print` within 200 chars prior, AND shell-LOLBin vocab in the backticked body because `SENSITIVE_PHP_KEYWORDS` is PHP-native and backticks run shell); amp-ratio clamp backfilled on PHP1/PHP3/PHP6 via module-local `_phpClipDeobfToAmpBudget` (32× raw / 8 KiB, `… [truncated]` marker reserved inside cap) — PHP1 gzinflate can legitimately expand 10-20× and trip the per-shell fuzz invariant `deobf > 32 × raw` without it.
