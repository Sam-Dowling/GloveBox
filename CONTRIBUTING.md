...
20. **Merged Timelines: `_sources` is the contract.** A Timeline view holds either `_sources=null` (single-file path) OR a `SourceRecord[]` registry with matching `_sourceOfRow` / `_sourceEnabledBitmap` of `length === store.rowCount`. Touching one without the others silently desyncs the filter pipeline. `TimelineView.fromSources(sources)` is the only constructor that wires all three; `_timelineAddFile(file)` + `_timelineRemountFromSources(sources, opts)` are the only mutators. Per-format canonical column projection lives in `src/app/timeline/timeline-mapper.js`; composite RowStore + time merge + bitmap in `src/app/timeline/timeline-composite.js`. Canonical schema is the 9-entry `TIMELINE_CANONICAL_COLS` (`__source`, `Timestamp`, `Host`, `User`, `EventID`, `Severity`, `Category`, `SourceIP`, `DestIP`) — short identifier-shape values only. Format identity is NOT canonical: the source filename in `__source` plus the per-chip format badge in the source-bar already convey it without burning grid width on a near-constant column. Wide-narrative columns (process command lines, message bodies, EVTX Event Data blobs, user-agent strings) stay on each source's native column plane where the original column name preserves the semantic. Per-source colour (chip swatch, breadcrumb popover dot, `__source` cell tint) is NOT a stored `SourceRecord` field — it is derived at render time from the source's CURRENT position in `view._sources` via `timelineSourceColor(idx)`, so chips and rows always share a hue and reshuffle in lockstep when a source is added or removed. Adding a new Timeline format needs: (a) tokeniser + column list in `timeline-parser-helpers.js`, (b) mapper entry in `timeline-mapper.js` (project ONLY short identifier-shape fields into canonicals; let everything else flow through as native columns), (c) register in `TIMELINE_MERGE_ELIGIBLE_KINDS` if merge-capable.
