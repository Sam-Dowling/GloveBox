...
| **Parser safety limits** | Centralised `PARSER_LIMITS` cap nesting depth (32), decompressed size (50 MB), per-entry compression ratio (100×), archive entry count (10 000), wall-clock parser runtime (60 s), and auto-YARA buffer size (32 MiB; above this, the on-load auto-scan is skipped and the analyst is told to scan via the YARA tab manually — the manual path is unrestricted because clicking the tab is an explicit opt-in to the latency cost). The 60 s runtime cap is a **whole-pipeline** wall-clock budget: `app-load.js` wraps the entire analyse-and-render pass in a single `ParserWatchdog.run()`, so it bounds total work across every parser invoked for a given file. Individual renderers do not have per-subsystem sub-budgets — a synchronous parser stuck in a tight loop will consume the whole budget without being interrupted mid-loop. |
