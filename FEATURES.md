...
| **Catch-all** | *Any file* — line-numbered text view (encoding auto-detect, 44-language syntax highlighting via vendored highlight.js v11.9.0 covering PowerShell / Batch / VBScript / bash / sh / zsh / JavaScript / TypeScript / Python / Perl / PHP / Ruby / Lua / Go / Rust / C / C++ / C# / Swift / Kotlin / Objective-C / VB.NET / Java / R / SQL / Markdown / YAML / JSON / XML / HTML / CSS / SCSS / Less / GraphQL / Dockerfile / Makefile / nginx / Apache / x86 Assembly / diff / INI / TOML / properties / plaintext; best-efforts code Format toggle + word-wrap toggle in the toolbar — Format works on files with a known extension/MIME and on pasted / extensionless content when a supported language is auto-detected with sufficient relevance, splits `;`-joined PowerShell / Bash one-liners into per-statement lines (respects strings, comments, here-strings, `$(…)` sub-expressions, `for (;;)` parens) and re-indents brace blocks; soft-wraps minified single-line files) or hex dump for binary data |
...
| **Recursive deobfuscation** | Peels nested Base64 / hex / Base32 / gzip / zlib / deflate / single-byte XOR layers and PowerShell / JavaScript / Bash / Python / PHP obfuscation shapes (variable indirection, string-array lookups, `String.fromCharCode` chains, token-spaced and concat-split identifiers, `${V:N:M}` parameter slicing, ANSI-C `$'\xNN'` quoting, `$(curl…\|sh)` pipe-to-shell, `eval(zlib.decompress(b64))` / `eval(marshal.loads(...))` Python carriers, `eval(gzinflate(base64_decode(...)))` PHP webshell decoder onions, p.a.c.k.e.r / aaencode / `Function(atob('...'))()` JS wrappers, PowerShell literal-free identifier reconstruction — `&(gcm i*x)` / `&(Get-Command i????e-rest*)` wildcard-to-cmdlet resolution against a curated sensitive table, `I<##>nv<##>oke-Expression` block-comment injection strip, `i''e''x` / `p""o""w""e""r""s""h""e""l""l` single-token quote interruption, `if ([bool]1254) { body }` truthy-guard flattening so the inner body's assignments become visible to the resolver). The sidebar shows the full nested-payload tree with hop pills and per-layer size deltas. Tolerates MIME / PEM / here-string wrapped Base64, Hex, and Base32 at any line width — a wrapped block is detected before the per-line scan and emitted as a single candidate, so a TVqQ/4D5A/eJw-prefixed payload broken across 50/60/64/72/76-char lines decodes identically to its unwrapped form (no misaligned-boundary garbage, no swarm of sub-floor per-line candidates). |
...
| **Deobfuscated script view** | When a dropper spreads one payload across multiple parallel obfuscation techniques (Base64 here, char-array there, CMD-obfuscation somewhere else — all feeding one `iex "$a $b $c"` line), Loupe splices every decoded span back into the source at its byte offset and produces a single "deobfuscated script" at the top of the Deobfuscation section. The deobfuscated body is re-analysed end-to-end: the IOC regex sweep surfaces indicators whose atoms were split across spans (a URL whose host and path lived in different findings), and rules tagged `applies_to = "decoded-payload"` run over the whole reconstructed body. Novel IOCs (visible only after stitching) are tagged `_fromReassembly` in the sidebar and carry `x_loupe_source = "reassembly"` / `[loupe-reassembly:<hash>]` through STIX and MISP exports. Language-agnostic — offset-splicing works for `.ps1` / `.bat` / `.js` / `.sh` / `.py` / `.php` / embedded `<script>` in `.hta` / `.svg` / `.html`. Bounded (64 spans, 4 MiB body, 5 % coverage floor, 2-finding minimum) so pathological input clips to `truncated: true` rather than crashing. Click **Analyse Deobfuscated Script** to drill into the reconstruction as a synthetic file. |
...
| **Nested-URL-in-query decoding** | ClickFix / redirector URLs that carry the real payload URL as a percent-encoded query parameter (e.g. `https://benign.example/r?a=https%3A%2F%2Fevil.com%2F…`) have the query decoded and re-scanned; any inner URL is surfaced as a medium-severity sibling with a `Nested URL (query param)` note so the real C2 host isn't hidden behind the outer redirector. |
...
| Capability | What you get |
|---|---|
| **Script scanning** | `.vbs`, `.ps1`, `.bat`, `.js`, `.cmd`, `.sh`, `.py`, `.pl`, `.php`, `.rb`, `.lua`, and similar standalone script types are scanned for dangerous execution patterns alongside full YARA matching covering PowerShell, JScript, VBS, CMD, Bash (11 rules: `/dev/tcp` reverse-shell, curl/wget live-fetch pipe-to-shell, bashrc / cron / sudoers / SSH-key persistence, IFS reassembly, heredoc-exec, dd pipe-to-shell, env -i masking), Python, and PHP (6 rules: webshell decoder onion, eval-superglobal one-liners with up to 3 sanitiser / decoder wrappers (`escapeshellarg`, `urldecode`, `base64_decode`, …), `preg_replace /e` modifier, variable-variables, `escapeshellarg`-on-superglobal webshell anti-pattern (option-injection), local-var taint flow (`$c = $_GET['x']; shell_exec($c);`)). JS obfuscators (aaencode / jjencode Hasegawa symbol carriers). Source is syntax-highlighted. |
...
| **Generic access log** (Pulse Secure · Ivanti Connect Secure · custom proxy / audit logs) | Leading timestamp (ISO 8601 · `YYYY-MM-DD--HH-MM-SS` · epoch s/ms) + space-delimited fields with CLF-style quoted runs | 8-col canonical schema (time · ip · tls_version · tls_cipher · request · bytes · referer · user_agent) for the TLS-access-log fingerprint; generic `time · field_2 · … · field_N` naming otherwise |
...
| **Merged multi-source Timelines** | Drop additional `.csv` / `.tsv` / `.evtx` / `.log` / structured-log files on top of an existing Timeline to **coalesce** them into one unified view. Every row is chronologically sorted regardless of source; queries, top-values, pivots, and EVTX Detections all apply across the merged set. A chip bar shows each source with its format pill, row count, stable colour, an on/off checkbox (Alt+1..9), and a 🗑 remove button. Each merged row carries canonical columns (`__source`, `__format`, `Timestamp`, `Host`, `User`, `Process`, `Message`, `EventID`, `Severity`, `Category`, `SourceIP`, `DestIP`) projected from the source's native fields by per-format mappers — so `source:events.csv`, `Host:web01`, `EventID:4624`, or `Severity:error` queries work the same across every format. Same-named columns in compatible sources FUSE into a single column; incompatible same-named columns get namespaced as `<source>·<col>`. PCAP and SQLite open standalone (not mergeable in v1). Soft cap 8, hard cap 16 sources. Mixed numeric-axis + wall-clock merges refused with a specific toast. Composite state is session-only. |
