...
| **Log files** (`.log` Apache CLF · syslog RFC 3164 · syslog RFC 5424 · Zeek TSV · CEF · LEEF · logfmt) | — | ◐ per-row | — | — (Timeline grid) |
| **CEF** (`.cef` Common Event Format / ArcSight — FortiGate · Palo Alto · Check Point · McAfee · Cisco · Trend Micro · F5 · Imperva · Juniper) | — | ◐ per-row | — | — (Timeline grid, syslog wrapper auto-stripped) |
| **LEEF** (`.leef` Log Event Extended Format / IBM QRadar — LEEF 1.0 tab-delimited · LEEF 2.0 custom delimiter incl. `\xHH` hex escape · Juniper · Trend Micro · McAfee · Symantec · Cisco) | — | ◐ per-row | — | — (Timeline grid, syslog wrapper auto-stripped) |
| **JSONL / NDJSON** (`.jsonl` `.ndjson` — newline-delimited JSON · AWS CloudTrail JSONL · container logs · fluentd / vector / Loki sinks) | — | ◐ per-row | — | — (Timeline grid) |
| **AWS CloudTrail** (`.json` `{"Records":[…]}` wrapped form) | — | ◐ per-row | — | — (Timeline grid, canonical column projection) |
...
| **Recursive deobfuscation** | Peels nested Base64, hex, Base32, gzip / zlib / deflate, JS `\xHH` hex escapes, char-array `String.fromCharCode` chains, reverse-string transforms, token-spaced obfuscation (`W r i t e - O u t p u t`), literal string-concat assembly, identifier-split-by-comments, single-byte XOR with key bruteforce, a PowerShell mini-evaluator that resolves `$var` / hashtable / `$env:` indirection so `&($a+$b) $c` style invocations surface as concrete commands, and a JS string-array resolver that defangs the obfuscator.io / `javascript-obfuscator` shape (string-array literal + indexer function + `eval` / `Function` / `setTimeout` sinks), recovering the sink-call payload through the same `_processCommandObfuscation` pipeline. The sidebar walks the full nested-payload tree so every layer (e.g. Base64 → gzip → PowerShell → Base64 → URL) gets its own section, with coloured hop pills showing the lineage and a size-delta row making unusual expansion / shrinkage obvious. |
...
Every CSV / TSV / EVTX / `.log` / `.jsonl` / `.ndjson` / `.cef` file — and SQLite browser history databases (Chrome / Edge / Firefox) — including extensionless drops identified by magic bytes or text sniffing — opens directly in Timeline: scrubber, stacked-bar chart, virtual grid, and per-column top-value cards on one page. No mode toggle, no threshold — these formats always route to Timeline. Generic (non-browser-history) SQLite databases use the tabbed-grid viewer.

`.log` files dispatch by content sniff: Apache / Nginx Common Log Format (the bracketed `[DD/Mon/YYYY:HH:MM:SS ±ZZZZ]` token) yields the canonical 9-column Combined / 7-column Common projection; Syslog RFC 3164 (`<PRI>MMM DD HH:MM:SS host program[pid]: message`) yields a 7-column projection (Timestamp · Severity · Facility · Host · Program · PID · Message) with the year inferred from the file's last-modified time using a 30-day future-roll heuristic; Syslog RFC 5424 (`<PRI>VER TIMESTAMP HOSTNAME APP-NAME PROCID MSGID SD MSG`) yields a 9-column projection (Timestamp · Severity · Facility · Host · App · ProcID · MsgID · StructuredData · Message) with native ISO 8601 timestamps and full structured-data block preservation (escaped `\"`, `\\`, `\]` inside quoted PARAM-VALUEs are honoured; multiple back-to-back `[...]` blocks are concatenated into the single StructuredData cell); Zeek TSV (lines beginning with `#separator \x09`) parses the inline `#fields` header to recover the per-`#path` schema (e.g. `conn.log` → 18 columns, `dns.log` → 23, `http.log` → 28), substitutes `(empty)` and `-` with blank cells per Zeek convention, and labels the view `Zeek (<path>)` with the histogram stack column auto-picked from a `#path` heuristic table (`conn`→`proto`, `dns`→`qtype_name`, `http`→`method`, `ssl`→`version`, `weird`→`name`, `files`→`mime_type`, `notice`→`note`); JSONL / NDJSON (newline-delimited JSON — `.jsonl`, `.ndjson`, or sniffed: ≥60% of the first 5 non-empty lines parse as JSON objects) locks the schema from the first valid record's flattened key set (nested objects flatten to dotted paths up to 8 levels deep, e.g. `client.ip`; arrays serialise in-place as JSON; max 256 columns) with a fixed `_extra` column always appended to capture any unknown keys from later records as a JSON-encoded sub-object — the histogram stack column is auto-picked from a candidate priority list (`level` · `severity` · `log.level` · `eventName` · `event_type` · `eventType` · `action` · `method` · `status` · `category`); AWS CloudTrail (records carrying `eventName` + `eventTime` keys, in either the JSONL form or the wrapped `{"Records":[…]}` single-document form — wrapped files are decoded, unwrapped, and re-emitted as JSONL bytes before dispatch) gets a dedicated route with the canonical column projection (eventTime · eventName · eventSource · awsRegion · sourceIPAddress · userIdentity.* · userAgent · eventID · eventType · errorCode · errorMessage · readOnly · managementEvent · …) seeded ahead of any record-driven schema discovery, the histogram pinned to `eventName`, and the format label set to `AWS CloudTrail` — service-specific blobs (`requestParameters.*`, `responseElements.*`) spill into the `_extra` column as a JSON sub-object so the headline grid stays scannable; CEF / Common Event Format (lines starting with `CEF:0|` or `CEF:1|`, with or without a leading syslog wrapper — `<134>Oct 15 22:14:15 host vendor: CEF:0|...` is auto-stripped) parses the 7 pipe-delimited header fields (Version · Vendor · Product · ProductVersion · SignatureID · Name · Severity, with `\\|` and `\\\\` escapes honoured) and the trailing `key=value` extension block (space-separated; values may contain spaces up to the next `<ident>=` boundary; `\\=`, `\\\\`, `\\n`, `\\r` escapes honoured), locks the extension schema from the first record's keys (cap 256 ext columns), spills unknown keys from later records into `_extra`, and pins the histogram stack column to `Severity` — covers FortiGate, Palo Alto PAN-OS, Check Point, McAfee ESM, Imperva WAF, Cisco ASA / FTD, Trend Micro, F5 ASM, Juniper SRX, and any other SIEM appliance speaking ArcSight CEF; LEEF / Log Event Extended Format (lines starting with `LEEF:1.0|` or `LEEF:2.0|`, with or without a leading syslog wrapper — `<134>Oct 15 22:14:15 host vendor: LEEF:1.0|...` is auto-stripped) parses the 5 pipe-delimited header fields (Version · Vendor · Product · ProductVersion · EventID, with `\|` and `\\` escapes honoured) and the trailing `key=value` extension block — LEEF 1.0 hard-codes the extension delimiter to TAB; LEEF 2.0 carries an additional 6th header field declaring the delimiter as a single literal character or a hex escape (`\xHH` or `0xHH`) which is consumed by the parser and **never** surfaced as an emitted column — locks the extension schema from the first record's keys (cap 256 ext columns), spills unknown keys from later records into `_extra`, and pins the histogram stack column to the first present candidate in `[sev, severity, cat, category]` (returning none if neither is in the locked schema) — covers IBM QRadar (the canonical producer), Juniper SRX, Trend Micro, McAfee, Symantec SEP, Cisco, and any other appliance speaking IBM LEEF; logfmt (a flat `key=value key="quoted value"` line format with no header — sniff threshold: ≥60 % of the first 5 lines yield ≥2 `<ident>=` pairs at line start or after whitespace, with the prefix before the first key occupying ≤60 % of the line) locks the column schema from the first valid record's key set (cap 256 cols), spills unknown keys from later records into a trailing `_extra` JSON sub-object, and pins the histogram stack column to the first present candidate in `[level, severity, lvl, msg, status, method]` — quoted values honour `\"` `\\` `\n` `\r` `\t` escapes; bare values run to the next ASCII whitespace; bare keys (no `=`) record the empty string. Covers Heroku router logs, Logrus, Hashicorp tools (Consul, Vault, Nomad), and the wider population of Go services that emit structured logs in this shape.
...
| **🌍 GeoIP + ASN enrichment** | Any column that's ≥80 % IPv4 addresses gets up to two automatic companion columns: `<col>.geo` (`Country/ISO2`, or `Country/ISO2/Region/City` if a richer geo MMDB is loaded) and `<col>.asn` (`Org Name (AS<n>)`, when an ASN MMDB is loaded). Lookups are 100 % offline: a bundled IPv4-country binary (≈830 KB, RIR-derived, refreshed monthly via cron PR) ships inside `loupe.html` itself, and ⚙ Settings → "GeoIP database" exposes **two independent slots** — Geo (Country / City / Region) and ASN (Autonomous System / Organisation) — each accepting a MaxMind-format `.mmdb` / `.mmdb.gz` (or DB-IP / IP2Location equivalents). Each slot is sniffed at upload time and rejected if the schema clearly belongs in the other slot. Both blobs are stored in IndexedDB (slot-namespaced) and never uploaded anywhere. Reserved / private / multicast ranges render empty. Enrichment is skipped for IP columns that already have geo / ASN data within ±3 columns (header tokens like `country`, `city`, `iso`, `asn`, `autonomous`, or values matching slash-delimited geo shapes / `^AS\d+`). Right-click any column header → 🌍 **Enrich IP** to force both providers on a column the auto-detector skipped (works on extracted IP columns from ƒx as well). Provider info (`bundled` vs `mmdb:<filename>`) and vintage are surfaced in the column tooltip. The same providers also enrich every IPv4 IOC across the rest of Loupe: each IP row in the **sidebar IOCs table** shows a muted `🌍 Country/ISO/…` line and (when an ASN MMDB is loaded) a `🛰 Org (AS<n>)` line; the **⚡ Summary** markdown report adds a fourth `Enrichment` column to its IOCs table; and the **IOC JSON** / **IOC CSV** clipboard exporters carry optional `geo` / `asn` fields (JSON) and `geo_country` / `geo_iso` / `geo_region` / `geo_city` / `asn` / `asn_org` columns (CSV). RFC1918 / loopback / link-local / multicast / CGNAT addresses are skipped silently. |
...
| **Drag-to-reorder columns (Timeline)** | Grab any column header in the Timeline grid and drag it left or right to rearrange the visible order; a 2 px accent-coloured bar shows the drop slot. The arrangement persists per-file across sessions. Sort, hide, top-values, GeoIP / ASN enrichment and right-click filters all continue to use the original column index, so behaviour stays stable regardless of the visible order. New auto-extracted ƒx columns append at the end (legacy behaviour); GeoIP / ASN enrichment columns land immediately next to the IPv4 source they enrich (geo first, then ASN). |
