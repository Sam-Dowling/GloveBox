...
| **🌍 GeoIP enrichment** | Any column that's ≥80 % IPv4 addresses gets an automatic `<col>.geo` companion column showing `Country/ISO2` (or `Country/ISO2/Region/City` if a richer database is loaded). Lookups are 100 % offline: a bundled IPv4-country binary (≈830 KB, RIR-derived, refreshed monthly via cron PR) ships inside `loupe.html` itself, and ⚙ Settings → "GeoIP database" lets you upload a MaxMind-format MMDB (Country / City / ASN) for a per-browser override — stored in IndexedDB, never uploaded anywhere. Reserved / private / multicast ranges render empty. Enrichment is skipped for IP columns that already have geo data within ±3 columns (header tokens like `country`, `city`, `iso`, or values matching slash-delimited geo shapes). Right-click any column header → 🌍 **Look up GeoIP** to force enrichment on a column the auto-detector skipped (works on extracted IP columns from ƒx as well). Provider info (`bundled` vs `mmdb:<filename>`) and vintage are surfaced in the column tooltip. |
