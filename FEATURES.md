...
| **Encoded content detection** | Scans for Base64, hex, Base32, gzip / zlib / deflate, JS `\xHH` hex escapes, char-array `String.fromCharCode` chains, reverse-string transforms (JS / PowerShell / Python `reversed()`), token-spaced obfuscation (`W  r  i  t  e  -  O  u  t  p  u  t`), literal string-concat assembly, identifier-split-by-comments (`/* */ console /* x */ . /* y */ log`) deobfuscation, single-byte XOR with key bruteforce on suspect blobs, and a PowerShell mini-evaluator that resolves variable / hashtable / `$env:` indirection so `&($a+$b) $c` style invocations surface as concrete commands. Decodes, classifies the payload (PE, script, URL list, …), extracts IOCs, and offers "Load for analysis" to drill into the decoded layer. |
...
| **Decode selection** | Click-drag any text in a content viewer to surface a floating "🔍 Decode selection" chip; clicking pipes just the highlighted bytes through the encoded-content analyser in **kitchen-sink bruteforce mode** — every decoder runs with thresholds dropped to the floor (3-char selections accepted), recursion deepened, and per-type caps lifted so deliberately small / noisy / heavily nested snippets still surface a full layer chain. Bruteforce additionally tries every Caesar shift (ROT-1…ROT-25), L=1/2/3/4-byte XOR keys with crib-driven scoring, and an interleaved-separator deobfuscator that strips fixed `\x00` / space / tab / `.` infill from `$\x00W\x00C\x00=\x00…`-style obfuscations. The loading overlay flips to a decode-themed phrase pool while the bruteforce runs. |
...
| **Subtle animated backdrop** | Per-theme drop-zone backdrop: an aperiodic Penrose rhombic tiling on Light, a slow wandering-node network on Dark, a whisper-low aperiodic Penrose rhombic tiling that rotates and breathes on Solarized, floating hearts on Mocha, floating kittens on Latte, and nothing at all on Midnight (pure-black stays pure-black for OLED). The animation is cosmetic only — it lives behind every chrome surface, hides the moment a file loads, and is suppressed entirely under `prefers-reduced-motion`. |
