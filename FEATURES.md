...
| **Merged multi-source Timelines** | Drop additional Timeline-eligible files on top of an existing Timeline to coalesce them into one chronologically-sorted view. Chip-bar toggles each source on/off; 10 canonical columns project across SIEM / audit / EDR vendor schemas so queries, pivots, and detections apply uniformly. |
