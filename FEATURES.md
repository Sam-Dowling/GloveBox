...
| **PE / executable analysis** | Parses PE32/PE32+ (EXE, DLL, SYS, `.xll`, etc.) — DOS/COFF/Optional headers, section table with entropy analysis, imports with suspicious API flagging (~140 APIs across injection, anti-debug, credential theft, networking categories), exports, resources, Rich header, string extraction; security feature detection (ASLR, DEP, CFG, SEH, Authenticode); surfaces high-signal "what is this binary?" heuristics — **Excel XLL add-in** (`xlAutoOpen`/`xlAutoClose`/`xlAutoAdd`/`xlAutoRemove`/`xlAutoRegister` exports, plus Excel-DNA managed-XLL detection), **compiled AutoHotkey** scripts (`>AUTOHOTKEY SCRIPT<` / `>AHK WITH ICON<` overlay markers), **Inno Setup / NSIS installers** (bundled script + payload extractor identifier), and **Go-compiled binaries** (`go.buildinfo`, `runtime.goexit`, `\xff Go buildinf:` markers with module path + version when present); 31 YARA rules for packers (UPX, Themida, VMProtect), malware toolkits (Cobalt Strike, Mimikatz, Metasploit), suspicious API patterns, and the new XLL / AutoHotkey / Go / Inno / NSIS heuristics |
...
| **ClickOnce deployment analysis** | Parses `.application` (deployment manifest) and `.manifest` (application manifest) XML — extracts assembly identity, description / publisher, deployment codebase + `deploymentProvider`, entry point, trust info + requested permission set, Authenticode-style `<Signature>` subject + thumbprint, and dependent assemblies / prerequisite codebases. Flags high-signal abuse patterns: **AppDomainManager hijacking** (malicious override of `appDomainManagerAssembly` / `appDomainManagerType` / `configurationFile`), **plain-HTTP deployment codebase** (MITM / downgrade risk), **FullTrust** permission requests, **dependent-assembly codebase pointing at disposable infrastructure** (free TLDs, tunneling services, paste sites). Non-ClickOnce `.manifest` XML files fall through to the plaintext renderer so side-by-side assembly manifests, SxS manifests, and vcpkg port manifests still render. 4 YARA rules. |
| **MSIX / APPX / App Installer analysis** | Parses `.msix` / `.msixbundle` / `.appx` / `.appxbundle` ZIP containers plus standalone `.appinstaller` XML — extracts package Identity (Name, Publisher DN, Version, Architecture), Properties (DisplayName, PublisherDisplayName, Description), Target Device Families, declared `<Capabilities>` split by risk tier (restricted `rescap:` such as `runFullTrust` / `broadFileSystemAccess` / `allowElevation`; device such as `webcam` / `microphone` / `location`; ordinary such as `internetClient`), Applications with Entry Points and their Extensions (`windows.fullTrustProcess` with invoked executable + parameters, `windows.startupTask` auto-start, `windows.appExecutionAlias` — flags aliases that squat common OS command names like `powershell.exe` / `cmd.exe` / `notepad.exe` — `windows.protocol` claims, `windows.fileTypeAssociation`, COM server/proxy/treatAs registrations, `windows.backgroundTasks`, services); enumerates bundled package contents for `.msixbundle` / `.appxbundle`; checks presence of `AppxSignature.p7x`, `AppxBlockMap.xml`, `AppxMetadata/CodeIntegrity.cat`; for `.appinstaller` parses `Uri`, `MainPackage` / `MainBundle`, `Dependencies`, and `UpdateSettings` (flags silent `ShowPrompt="false"` forced auto-updates). 9 dedicated YARA rules in `msix-threats.yar` (`MSIX_Capability_RunFullTrust`, `MSIX_Capability_BroadFileSystem`, `MSIX_Extension_FullTrustProcess`, `MSIX_Extension_StartupTask`, `MSIX_AppExecutionAlias_CommonName`, `MSIX_AppInstaller_HTTP`, `MSIX_AppInstaller_Suspicious_TLD`, `MSIX_AppInstaller_Silent_AutoUpdate`, `MSIX_Protocol_Claim_Common`). Inner files (nested `.msix`, `.exe`, `.dll`, scripts) are clickable for recursive analysis. |
