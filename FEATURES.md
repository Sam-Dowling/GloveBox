...
| **Encoded content detection** | Scans for Base64, hex, Base32, gzip / zlib / deflate, JS `\xHH` hex escapes, char-array `String.fromCharCode` chains, reverse-string transforms (JS / PowerShell / Python `reversed()`), token-spaced obfuscation (`W  r  i  t  e  -  O  u  t  p  u  t`), literal string-concat assembly, identifier-split-by-comments (`/* */ console /* x */ . /* y */ log`) deobfuscation, and single-byte XOR with key bruteforce on suspect blobs. Decodes, classifies the payload (PE, script, URL list, …), extracts IOCs, and offers "Load for analysis" to drill into the decoded layer. |
...
| **Decode selection** | Click-drag any text in a content viewer to surface a floating "🔍 Decode selection" chip; clicking pipes just the highlighted bytes through the encoded-content analyser with aggressive-mode thresholds (lower entropy / length cuts) so deliberately small or noisy snippets still surface a layer chain. |
