...
| **TLS callbacks + entry-point sanity** | Walks `IMAGE_DIRECTORY_ENTRY_TLS → AddressOfCallBacks`, lists every callback (VA / RVA / file-offset / containing section) with clickable hex-dump drill-down, and flags anomalous entry points (outside any section, or in a W+X section). Callbacks in W+X sections and callbacks paired with anti-debug imports escalate to high (T1546.009 / T1027.*). |
...
| **Binary Pivot triage card** | Above-the-fold summary card at the top of every PE / ELF / Mach-O view with the fields an analyst hits first — SHA-256 / SHA-1 / MD5, import-shape hash (imphash / telfhash / Mach-O import hash), RichHash or SymHash, signer CN / Team ID or "unsigned", compile timestamp with a "faked?" flag (1970 epoch, `0xFFFFFFFF`, Borland TLINK default, future-dated, pre-2000, and the Feb 2006 reproducible-build sentinel), entry-point section plus anomaly (orphan / W+X / non-code), overlay Y/N with first-bytes magic, and packer verdict. Identical layout across the three formats. |
| **Triage-first verdict band** | One-line human verdict (e.g. *"Unsigned PE32+ DLL · UPX-packed · orphan EP · 40 KB overlay · 8 capabilities"*) + coloured tier chip (clean / low / medium / high / critical) + 0–100 risk score, rendered above the Pivot card on every PE / ELF / Mach-O view. |
| **Anomaly ribbon** | Short row of severity-coloured chips (orphan EP, W+X section, packed, RWX segment, exec-stack, ad-hoc-signed, dangerous entitlements, world-writable `@rpath`, …) under the verdict band. Anomalous Tier-C reference cards (Sections, Load Commands, Entitlements, Overlay, …) auto-expand when a matching chip fires. |
...
| **PE resource drill-down** | Walks the three-level PE resource directory (type → name → language) and lists every leaf with its size and first-bytes magic. RCDATA / HTML / MANIFEST / named-type leaves — plus any leaf whose first bytes sniff as a known format — become clickable and open in a fresh renderer dispatch. Embedded PE / ELF / Mach-O payloads flag as high (T1027.009 — Embedded Payloads); embedded archives (ZIP / 7z / RAR / gzip / CAB) in RCDATA-style slots flag as medium; large unrecognised high-entropy blobs (> 64 KB, entropy > 7.2) in the same slots flag as medium (T1027.002). Icons, cursors, fonts, string / message tables, menus, dialogs, accelerators and version info are skipped. |
| **Categorised binary strings** | The extracted PE / ELF / Mach-O string corpus is classified into mutex names (`Global\…` / `Local\…`), Windows named pipes (`\\.\pipe\…`), PDB paths, user-home / build-tree paths (`C:\Users\…`, `/home/…`, `/Users/…`), registry keys, and Rust panic source paths (`panicked at 'msg', src/foo.rs:LL:CC`) — each surfaced as its own sidebar IOC with the appropriate type. Mutex names cluster sibling samples; PDB + Rust-panic paths leak build-host attribution on the very first triage pass. |
| **.NET CLR detection** | PEs carrying `IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR` are recognised as managed .NET assemblies — the CLR header is parsed for the runtime version string (from the BSJB metadata root), the IL-only / mixed-mode / strong-name / prefer-32-bit flags, and the entry-point token. Surfaced as a dedicated **🔷 .NET CLR Header** card plus an `IOC.PATTERN` (T1059.005) so managed samples cluster as a distinct execution vector. Mixed-mode (C++/CLI) assemblies add a secondary flag for the native code path. |
| **Export-anomaly flags** | Library exports (PE DLL, Mach-O MH_DYLIB, ELF .so with `DT_SONAME`) are checked for three well-known triage signals. **DLL side-loading hosts** — a DLL whose basename matches a canonical T1574.002 target (`version.dll`, `winmm.dll`, `uxtheme.dll`, `winhttp.dll`, `dbghelp.dll`, `cryptbase.dll`, VC-runtime redistributables, `libcurl.dll`, `libssl-1_1.dll`, …) is flagged high-severity, regardless of its exports. **Forwarded / proxy-DLL exports** — PE exports whose RVA falls inside the export directory are resolved to their `OtherDll.FuncName` target and each non-platform forwarder is surfaced medium; the Mach-O equivalent (`LC_REEXPORT_DYLIB`) is reported under the same banner. **Ordinal-only exports** — a PE where ≥ 50 % of exports have no names (and ≥ 4 are ordinal-only) flags medium as a packer / loader tell (T1027). |
...
| **Keyboard shortcuts** | `S` sidebar · `Y` YARA dialog · `N` Nicelists · `T` Timeline mode · `,` Settings · `?` / `H` Help · `F` search document · `Ctrl+C` copy raw file (when nothing is selected) · `Ctrl+V` paste file for analysis · `Esc` close dialog / clear search. **Archive browser:** `/` focus filter · `↑ ↓` navigate rows · `← →` collapse / expand folder · `Enter` / `Space` open selected file. |
...
| **📈 Timeline mode** | Dedicated top-level mode for CSV / TSV / EVTX — scrubber, stacked-bar chart, virtual grid and per-column top-value cards stack on one page with no sidebar. Drop a large CSV / TSV or any EVTX to auto-switch in (toggle in ⚙ Settings). Chips AND-filter values, shift-click NOTs, ctrl-click replaces. Press `T` or click 📈 to toggle. |
...
- [`pe/tls-callback.exe`](examples/pe/tls-callback.exe) — minimal PE32 with one TLS callback registered via `IMAGE_DIRECTORY_ENTRY_TLS`; exercises the Phase-4 parser, the clickable TLS Callbacks card, and the medium-severity **T1546.009** IOC
- [`pe/rcdata-dropper.exe`](examples/pe/rcdata-dropper.exe) — minimal PE32 with a second minimal PE stashed as an `RT_RCDATA` resource leaf; exercises the resource drill-down table, click-through re-dispatch, and the high-severity **T1027.009** embedded-payload IOC
