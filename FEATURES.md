...
| **Reassembled script view** | When a dropper spreads one payload across multiple parallel obfuscation techniques (Base64 here, char-array there, CMD-obfuscation somewhere else — all feeding one `iex "$a $b $c"` line), Loupe splices every decoded span back into the source at its byte offset and produces a single stitched "reassembled script" at the top of the Deobfuscation section. The stitched body is re-analysed end-to-end: the IOC regex sweep surfaces indicators whose atoms were split across spans (a URL whose host and path lived in different findings), and rules tagged `applies_to = "decoded-payload"` run over the whole reconstructed body. Novel IOCs (visible only after stitching) are tagged `_fromReassembly` in the sidebar and carry `x_loupe_source = "reassembly"` / `[loupe-reassembly:<hash>]` through STIX and MISP exports. Language-agnostic — offset-splicing works for `.ps1` / `.bat` / `.js` / `.sh` / `.py` / `.php` / embedded `<script>` in `.hta` / `.svg` / `.html`. Bounded (64 spans, 4 MiB stitched text, 5 % coverage floor, 2-finding minimum) so pathological input clips to `truncated: true` rather than crashing. Click **Load stitched script for analysis** to drill into the reconstruction as a synthetic file. |
