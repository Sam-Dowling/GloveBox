...
extendApp({
  ...
  _renderSidebar(fileName, analyzer) {
    ...
    // (No sidebar-side reasons panel: PE/ELF/Mach-O viewers already render
    // the same `findings.riskReasons` rows in the in-content verdict band
    // via `BinaryTriage.renderReasonsPanel`. Duplicating it in the sidebar
    // was redundant on the only surfaces where it actually appeared. The
    // structured data is still surfaced to clipboard analysis and to the
    // e2e test snapshot — see `_buildAnalysisMarkdown` and
    // `_testApiDumpFindings` — so non-UI consumers continue to see the
    // full audit trail.)
