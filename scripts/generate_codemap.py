#!/usr/bin/env python3
"""
scripts/generate_codemap.py — Generates CODEMAP.md for AI coding agents.
...
Usage:  python scripts/generate_codemap.py          # writes CODEMAP.md
        python scripts/generate_codemap.py --json   # writes codemap.json (machine-readable)

Also runs as `python make.py codemap`.
"""
...
# scripts/generate_codemap.py → repo root is one level up.
...
def generate_markdown(js_infos, css_infos, yar_infos, cross_refs,
    ...
    w('# 🗺️ Loupe Code Map')
    w('')
    w(f'> Auto-generated by `scripts/generate_codemap.py` on {datetime.now().strftime("%Y-%m-%d %H:%M")}.')
    w('> **Re-run `python scripts/generate_codemap.py` after any code changes.**')
    w('')
    ...
    w('`python scripts/generate_codemap.py` to regenerate after code changes.*')
